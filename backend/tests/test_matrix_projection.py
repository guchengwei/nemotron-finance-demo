...
class TestSpreadScores:
    ...
    def test_all_identical_preserves_shared_value(self):
        raw = [4.0, 4.0, 4.0]
        spread = spread_scores(raw)
        assert spread == [4.0, 4.0, 4.0]

    def test_all_identical_high_values_preserve_high_side(self):
        raw = [5.0, 5.0, 5.0]
        spread = spread_scores(raw)
        assert spread == [5.0, 5.0, 5.0]
