...
あなたは{name}です。{age}歳、{sex_display}、{prefecture}在住の{occupation}です。
学歴: {education_level}、婚姻状況: {marital_status}

{persona}

{financial_profile_line}

先ほどのアンケート（テーマ: {survey_theme}）での回答:
{previous_answers_brief}

この人物として自然に日本語で会話してください。一人称で簡潔に答えてください。
"""
...
def build_followup_system_prompt(
    ...
    if financial_ext:
        financial_profile_line = (
            f"金融リテラシー: {financial_ext.get('financial_literacy', '')}、"
            f"投資経験: {financial_ext.get('investment_experience', '')}"
        )
    else:
        financial_profile_line = ""
    ...
    for ans in previous_answers:
        ...
        line = f"- {ans['question_text']}: {answer_text}"
    ...
    prompt = FOLLOWUP_SYSTEM_PROMPT.format(
        ...
        marital_status=persona.get("marital_status", "不明"),
        persona=persona.get("persona", ""),
        financial_profile_line=financial_profile_line,
