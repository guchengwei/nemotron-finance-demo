/**
 * Screenshot capture spec for README demo images.
 *
 * Runs against the Vite preview (mock mode — no real backend required).
 * All API calls are intercepted via page.route() so no vLLM is needed.
 *
 * Output: docs/screenshots/01-filter-panel.png  … 04-report-matrix.png
 *
...
const richFilters: FiltersResponse = {
  sex: ['男', '女'],
...
const demoPersonas: Persona[] = [
  {
    ...
    education_level: '大学卒',
    marital_status: '既婚',
    persona: 'ITマネージャーとして活躍する中堅サラリーマン。デジタルツールへの親和性が高く、資産形成に積極的。',
    professional_persona: 'ITマネージャー',
    cultural_background: '都市部',
    skills_and_expertise: 'IT・プロジェクト管理',
    hobbies_and_interests: '投資・ゴルフ',
    career_goals_and_ambitions: '早期リタイアを目指した資産形成',
  },
  {
    uuid: 'p2',
    name: '鈴木美咲',
    ...
    marital_status: '未婚',
    persona: 'UIデザイナーとして独立。収入変動があるため自動資産管理に強い関心を持つ。',
    professional_persona: 'UIデザイナー',
    cultural_background: '都市部',
    skills_and_expertise: 'デザイン・マーケティング',
    hobbies_and_interests: 'カフェ巡り・読書',
    career_goals_and_ambitions: '安定した収入基盤の確立',
  },
  {
    uuid: 'p3',
    name: '佐藤誠',
    ...
    education_level: '大学卒',
    marital_status: '既婚',
    persona: '製造業で自営業を営む経営者。アナログ志向が強くデジタル化には慎重なスタンス。',
    professional_persona: '中小企業経営者',
    cultural_background: '地方都市',
    skills_and_expertise: '製造・経営管理',
    hobbies_and_interests: '釣り・野球観戦',
    career_goals_and_ambitions: '事業承継と老後の安定',
  },
  {
    uuid: 'p4',
    name: '高橋亮子',
    ...
    education_level: '大学卒',
    marital_status: '未婚',
    persona: 'マーケティング職の若手社員。つみたてNISAを始めたばかりで長期資産形成に積極的。',
    professional_persona: 'マーケティング担当',
    cultural_background: '都市部',
    skills_and_expertise: 'マーケティング・SNS運用',
    hobbies_and_interests: '旅行・料理',
    career_goals_and_ambitions: '将来のための長期的な資産形成',
  },
]

// ── SSE builders ───────────────────────────────────────────────────────────

function buildSurveySSE(): string {
  const questions = [
    'このデジタル資産管理サービスへの関心度と利用意向を教えてください。',
    '導入する際の障壁や懸念点があれば具体的にお聞かせください。',
  ]
  const answers: Record<string, Array<{ answer: string; score: number }>> = {
    p1: [
  ...
  for (const p of demoPersonas) {
    ...
    for (let qi = 0; qi < questions.length; qi++) {
      events.push({
        ...
        data: { persona_uuid: p.uuid, question_index: qi, answer: answers[p.uuid][qi].answer, score: answers[p.uuid][qi].score },
      })
    }
...
function buildMatrixSSE(): string {
  const axisConfig = {
  ...
  const scoreTable = scoredPersonas.map((p) => ({
    persona_id: p.persona_id, name: p.name, x_score: p.x_score, y_score: p.y_score,
    industry: p.industry, age: p.age, quadrant_label: p.quadrant_label,
  }))
  const events: Array<{ event: string; data: unknown }> = [
...
test('capture demo screenshots for README', async ({ page }) => {
  await page.setViewportSize({ width: 1280, height: 800 })

  // Mock all API endpoints (no real backend needed)
  ...
  await page.route('**/api/personas/filters', (route) =>
    route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(richFilters) })
  )
  await page.route('**/api/history', (route) =>
    route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ runs: [] }) })
  )
  await page.route('**/api/personas/count**', (route) =>
    route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ total_matching: 4872 }) })
  )
  await page.route('**/api/personas/sample**', (route) =>
    route.fulfill({
      status: 200, contentType: 'application/json',
      body: JSON.stringify({ total_matching: 4, sampled: demoPersonas }),
    })
  )
  await page.route('**/api/survey/run', (route) =>
    route.fulfill({ status: 200, contentType: 'text/event-stream', body: buildSurveySSE() })
  )
  await page.route('**/api/report/generate', (route) =>
    route.fulfill({
      ...
      body: JSON.stringify({
        run_id: RUN_ID,
        overall_score: 4.0,
        group_tendency: '全体的に高い関心を持ちながらも、セキュリティと操作性に関する懸念が障壁となっています。若年層とデジタルネイティブ層は即時採用ポテンシャルが高く、シニア層は丁寧な導線設計が必要です。',
        conclusion: 'デジタル資産管理サービスは特に20〜30代のデジタルネイティブ層に強い訴求力があります。セキュリティの透明化とAI機能の充実を軸に展開することで、採用率の最大化が期待できます。',
        top_picks: [],
      }),
    })
  )
  await page.route('**/api/report/matrix', (route) =>
    route.fulfill({ status: 200, contentType: 'text/event-stream', body: buildMatrixSSE() })
  )
  ...
  await page.screenshot({ path: '../docs/screenshots/01-filter-panel.png' })

  // ── 02: Survey Config ───────────────────────────────────────────────────

  await page.getByPlaceholder('カスタム').fill('4')
  await page.getByRole('button', { name: 'ペルソナを抽出 (4名)' }).click()
  ...
  await page.getByTestId('survey-theme-input').fill('デジタル資産管理サービスの顧客適合性調査')
  await page.waitForTimeout(300)
  await page.screenshot({ path: '../docs/screenshots/02-survey-config.png' })
  ...
  await page.waitForTimeout(300)
  await page.screenshot({ path: '../docs/screenshots/03-survey-runner.png' })
  ...
  await page.screenshot({ path: '../docs/screenshots/04-report-matrix.png' })
})
